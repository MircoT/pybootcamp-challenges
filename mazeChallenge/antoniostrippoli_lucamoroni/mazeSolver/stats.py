...
def _preprocess_data_hist(colors_xy: tuple):
    """
    ...
    return (x_label, x_red, x_green, x_blue, x_white,
            y_label, y_red, y_green, y_blue, y_white)


def _plot_axes(ax1, ax2, colors_xy, label="", width=0.2):
    """
    Internal function that plots colors distribution on given axes
    Width is used to determine the width of each bar
    """
    # Preprocess data
    (x_label, x_red, x_green, x_blue, x_white,
     y_label, y_red, y_green, y_blue, y_white) = _preprocess_data_hist(
        colors_xy)
...
def plot_colors_xy_dist(maze: "Maze", past_maze: "Maze"):
    ...
    if past_maze:
        ...
        _plot_axes(
            ax21, ax22, (past_maze.colors_x, past_maze.colors_y),
            label="PAST maze color distribution on "
        )
    else:
    ...
    _plot_axes(
        ax11, ax12, (maze.colors_x, maze.colors_y),
        label="CURRENT maze color distribution on "
    )
    plt.show()
