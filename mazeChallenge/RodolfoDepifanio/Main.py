...
def Assegnatrice_Comando(x0, y0, x1, y1, command):
    """assegnati due punti (attuale e da raggiugere)
    da restitusce action opportuna"""
...
def esploratore(path, Nlim):
    ...
    # inizializzo per avere il primo stato
    res = json.loads(mazeClient.send_command(command.MOVE_DOWN))
    ...
    while k < Nlim:  # a tot caselle ripetute considero il labirinto esplorato
        ...
        vicini = []
        # elimino gli spostamenti impossibili ==> lista vicini
        ...
        for i in vicini:
            if [i[0], i[1], i[2]] not in caselle_fatte:
                ...
                k = 0
                break
        # se non ci sono, controllo quindi se posso tornare su una vecchia
        # che non sia nella lista chiusa o quella da dove vengo
        if flag == 0:
            for i in vicini:
                if x_p != [i[0], i[1]] and [i[0], i[1]] not in caselle_chiuse:
                    ...
                    flag = 1
                    break
        # se non ci sono, torno quella da dove venivo e inserisco nella
        # lista di caselle chiuse quella attuale
        if flag == 0:
            print('Casella cieca')
            x_s = x_p
...
def analisi(dati):
    ...
    # distribuzione dei punti cololorati
    maze['frequenza'] = 1 - maze['colore'].str.count('w')
    n = [0]*3  # vettore rgb
    n[0] = maze['colore'].str.count('r').sum()
    n[1] = maze['colore'].str.count('g').sum()
    n[2] = maze['colore'].str.count('b').sum()
    # distribuzione sulle x del primo
    fxc = maze.groupby(maze['x'])['frequenza'].sum()
...
def main():
    ...
    while(flag):
        try:
            ...
            # distribuzione
            fig_2, (ax1, ax2) = plt.subplots(
                1, 2, figsize=(10, 4), gridspec_kw={
                    'width_ratios': [
                        len(fxc1)/(len(fxc1)+len(fyc1)),
                        len(fyc1)/(len(fxc1)+len(fyc1))
                    ]
                }
            )
    ...
    while(flag):
        try:
            ...
            fig_1, (ax3, ax4) = plt.subplots(
                1, 2, figsize=(0.4*lfxc, 0.2*lfyc))
